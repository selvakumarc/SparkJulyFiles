...
object LearningScala1 {
  ...
  println(hello)                //> Hola!
   ...
   /*Notice how Scala defines things backwards from other 
     languages - you declare the name, then the type.*/
  ...
  println(helloThere)                             //> Hola! There!
   ...
   /* 1:  One key objective of functional programming is to use 
     immutable objects as often as possible.
     2: Try to use operations that transform immutable objects into a 
     new immutable object.
     3: For example, we could have done the same thing like this:*/
  ...
  val bigNumber  = 1234567890l              //> bigNumber  : Long = 1234567890
